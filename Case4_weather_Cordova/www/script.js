$(document).ready(function() {
	...
	function getTaiwanWeather(city_name) {
		...
		var Your_Weather_API_key = "CWB-FCBEA340-4E19-4A18-AAE2-D00CF6F7D246";  //IMPORTANT, replace it with your weather API Authkey 中央氣象局授權碼
		...
		.done(function(arr) {
			...
			var weather_1 = arr.records.locations[0].location[0].weatherElement[6].time[0].elementValue[1].value.substr(1);
			var weather_value_1 = arr.records.locations[0].location[0].weatherElement[6].time[2].elementValue[1].value.substr(1);
			var weather_value_2 = arr.records.locations[0].location[0].weatherElement[6].time[4].elementValue[1].value.substr(1);
			var weather_value_3 = arr.records.locations[0].location[0].weatherElement[6].time[6].elementValue[1].value.substr(1);
			...
			//console.log(weather_1,weather_value_1,weather_value_2,weather_value_3);
			var skycons = new Skycons({"color": "#A9DD9B"});
			skycons.set("weather-icon", weather_dict[weather_1]);
